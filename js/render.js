...
// Depends on: data.js (CATS, STATUS, STATUS_ORDER, fmt, esc, stars, daysLeft, vendorsFor, allCats)
// ─────────────────────────────────────────────────────────
...
function renderVendorCard(v, accent) {
  ...
  var rmk = v.remarks
    ...
    : '';
  var phone = v.phone ? '<div class="vc-phone">📞 ' + esc(v.phone) + '</div>' : '<div></div>';
...
function renderHome(st) {
  ...
  var tiles = cats.map(function(c) {
    ...
    if (isBooked) {
      ...
      tileClass  = 'st-active';
      tileStyle  = 'style="background:' + c.color + '"';
    ...
    return '<div class="cat-tile ' + tileClass + '" ' + tileStyle + ' data-a="cat" data-id="' + c.id + '">' +
      ...
      '<div>' +
        ...
        '<div class="tile-hint">' + tileHint + '</div>' +
      '</div>' +
    '</div>';
  }).join('');
  ...
  var addTile = '<div class="cat-tile st-add" data-a="addcat">' +
    ...
    '<div>' +
      ...
      '<div class="tile-hint">Custom vendor type</div>' +
    '</div>' +
  '</div>';
  ...
  return '<div>' +
    '<div class="home-header">' +
      ...
      '<div class="wedding-chips">' +
        ...
        '<div class="chip">⏳ ' + days + ' days to go</div>' +
        '<div class="chip">✅ ' + bookedCats + ' booked</div>' +
      '</div>' +
      '<div class="header-progress">' +
        '<div class="hp-bar"><div class="hp-fill" style="width:' + pctDone + '%"></div></div>' +
        '<div class="hp-label">' + bookedCats + ' of ' + cats.length + ' categories confirmed · ' + activeCats + ' in progress · ' + pendingCats + ' pending</div>' +
      '</div>' +
    '</div>' +
    '<div class="content">' +
...
function renderForm(st) {
  ...
  var existing = st.vendorId
    ...
    : null;
  var v = existing || { name:'', phone:'', location:'', quotedPrice:'', packageDetails:'', status:'exploring', rating:0, remarks:'' };
  var isEdit = !!existing;
  ...
  return '<div>' +
    ...
    '<div class="form-section">' +
      ...
      '<textarea id="f-pkg" class="form-input" style="margin-bottom:0" placeholder="e.g. 8 hrs, 2 photographers, 500 edited photos, same-day reel, album">' + esc(v.packageDetails) + '</textarea>' +
    '</div>' +
    ...
    '<div class="form-section">' +
      ...
      '<textarea id="f-remarks" class="form-input" style="margin-bottom:0" placeholder="e.g. Very professional. Great portfolio. Can negotiate ₹5K. Recommended by Sharma uncle.">' + esc(v.remarks) + '</textarea>' +
    '</div>' +
...
function renderDashboard(st) {
  ...
  var budgetCard = '<div class="budget-card">' +
    ...
    '<div class="bo-input-row">' +
      '<div>' +
        '<div class="bo-field-label">Total Budget (₹)</div>' +
        '<input class="bo-input" id="b-budget" type="number" value="' + (budget || '') + '" placeholder="e.g. 1500000">' +
      '</div>' +
      '<div>' +
        '<div class="bo-field-label">Amount Paid (₹)</div>' +
        '<input class="bo-input" id="b-paid" type="number" value="' + (paidAmt || '') + '" placeholder="e.g. 50000">' +
      '</div>' +
    '</div>' +
    barSection +
    '<div class="bo-stats-row">' +
      '<div class="bo-stat">' +
        '<div class="bos-label">Committed</div>' +
        '<div class="bos-val count-up" data-target="' + bookedAmt + '" data-prefix="₹">' + (bookedAmt ? fmt(bookedAmt) : '—') + '</div>' +
      '</div>' +
      '<div class="bo-stat">' +
        '<div class="bos-label">Still to Pay</div>' +
        '<div class="bos-val red">' + (balanceDue ? fmt(balanceDue) : '—') + '</div>' +
      '</div>' +
      '<div class="bo-stat">' +
        '<div class="bos-label">Remaining</div>' +
        '<div class="bos-val green">' + (remaining !== null ? fmt(remaining) : '—') + '</div>' +
      '</div>' +
    '</div>' +
  '</div>';
  ...
  return '<div>' +
    '<div class="dash-header">' +
      ...
      '<p>Nishant ki Shadi · Dec 13–14, 2026</p>' +
    '</div>' +
    '<div class="content">' +
    ...
    '<div class="stat-grid">' +
      '<div class="stat-card">' +
        ...
        '<div class="stat-value count-up" style="color:#1B5E20;font-size:' + (bookedAmt >= 100000 ? '22px' : '28px') + '"' +
          ' data-target="' + bookedAmt + '" data-prefix="₹">' + (bookedAmt ? fmt(bookedAmt) : '—') + '</div>' +
    ...
    (breakdown.length
      ...
      : '<div class="empty"><div class="eicon">📊</div><p>Add vendors to see the breakdown here</p></div>') +

    '<div class="stat-card">' +
      '<div class="stat-label">All Vendors · ' + allV.length + ' total</div>' +
      '<div class="status-summary">' + ssPills + '</div>' +
    '</div>' +
